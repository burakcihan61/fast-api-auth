...
@pytest.mark.asyncio
async def test_trigger_email_task_success(
    ...
    with patch("app.tasks.sample_tasks.send_email_task.delay") as mock_delay:
        ...
        response = await client.post(
            "/api/v1/tasks/email",
            json={"email": "test@example.com", "subject": "Test Subject", "message": "Hello World"},
...
@pytest.mark.asyncio
async def test_trigger_report_task_success(
    client: AsyncClient, normal_user_token_headers: dict[str, str]
):
...
async def test_get_task_status(client: AsyncClient, normal_user_token_headers: dict[str, str]):
    ...
    with patch("app.api.v1.tasks.AsyncResult") as mock_result:
        ...
        response = await client.get("/api/v1/tasks/test-task-id", headers=normal_user_token_headers)
